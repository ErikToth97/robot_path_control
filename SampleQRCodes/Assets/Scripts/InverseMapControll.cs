...
public class InverseMapControll : MonoBehaviour
{
    ...
    private void controllRobot(float[] rotations)
    {
        ...
        if((joints[5].position - endEffector.transform.position).magnitude > 0.1+ tcpdist)
        {
            rotations[3] += 180;
            joints[3].localRotation = Quaternion.Euler(rotations[3] * rotAxis[3][0], rotations[3] * rotAxis[3][1], rotations[3] * rotAxis[3][2]);
        }
    }
